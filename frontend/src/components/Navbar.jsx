      ...
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-4 py-3">
        ...
        <nav className="flex items-center gap-2">
            ...
            <div className="relative" ref={cartRef}>
              ...
              <div
                className={`fixed right-2 top-18.5 z-50 w-64 max-w-[90vw] rounded-lg border border-[#f4c23d]/30 bg-[#1e2436] px-3 pb-3 pt-2 text-white shadow-xl transition-all duration-200 ${
              ...
              >
                ...
                <Button type="button" size="sm" className="mt-2.5 h-8 w-full text-[11px] uppercase tracking-[0.08em]" onClick={proceedToOrder}>
                  Proceed to Order
                </Button>
              </div>
                    ...
                    {cartItems.map((item, index) => (
                      <div key={item.id || `${item.name}-${index}`} className="flex items-center justify-between rounded-md border border-slate-200 px-3 py-2">
                        <div>
                          ...
                          <p className="text-[12px] text-slate-600">{item.rice} · ₱{Number(item.price || 0).toFixed(2)}</p>
                        </div>
                        <button
                          type="button"
